...
class AnimalRepository {
    ...
    fun atualizarAnimal(
        ...
        val animalValues = mapOf(
            ...
            "raca" to animal.raca,
            "idade" to animal.idade,
            "sexo" to animal.sexo,
            "porte" to animal.porte,
            "status" to animal.status,
            "comportamento" to animal.comportamento,
            "fotoUrl" to animal.fotoUrl,
            "descricao" to animal.descricao
        )

        database.child(animal.id)
            .updateChildren(animalValues)
            .addOnSuccessListener { onSuccess() }
            .addOnFailureListener { onFailure(it) }
    }
